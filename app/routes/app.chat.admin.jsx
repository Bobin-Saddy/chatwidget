...
// --- ARTISAN ICON SET (Added Paperclip and X) ---
const Icons = {
  Send: () => (
...
export default function NeuralChatAdmin() {
  ...
  const handleReply = (text = null) => {
    ...
    if ((!finalMsg.trim() && !selectedFile) || !activeSession) return;
    ...
    const newMessage = { 
      ...
      shop: currentShop,
      hasFile: !!selectedFile // Just for local UI logic
    };
  ...
  return (
    <div style={{ 
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 15px 20px' }}>
          {filteredSessions.map(session => (
            <div key={session.sessionId} onClick={() => loadChat(session)}
              style={{
                padding: '20px', borderRadius: '22px', cursor: 'pointer', marginBottom: '10px',
      ...
      <div style={{ flex: 1, borderRight: '1px solid #ccc', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {activeSession ? (
          <>
              ...
              {messages.map((msg, i) => (
                <div key={i} style={{ alignSelf: msg.sender === 'admin' ? 'flex-end' : 'flex-start', maxWidth: '75%' }}>
                  <div style={{ 
                    ...
                    fontSize: '15px', background: msg.sender === 'admin' ? accentColor : 'white',
            ...
            <div style={{ padding: '20px 40px', borderTop: '1px solid rgb(253, 250, 245)' }}>
              {/* File Preview before sending */}
              {selectedFile && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px', background: '#fff1e6', padding: '8px 15px', borderRadius: '10px', width: 'fit-content' }}>
                   <span style={{ fontSize: '12px', color: '#8b5e3c', fontWeight: '700' }}>📎 {selectedFile.name}</span>
                   <button onClick={() => setSelectedFile(null)} style={{ border: 'none', background: 'none', cursor: 'pointer', color: '#8b5e3c' }}><Icons.X /></button>
                </div>
              )}

              <div style={{ display: 'flex', gap: '8px', marginBottom: '15px' }}>
                {['Happy to help!', 'Checking your order'].map(t => (
                  <button key={t} onClick={() => handleReply(t)} style={{ padding: '8px 16px', borderRadius: '100px', border: '1px solid #f1ece4', fontSize: '11px', fontWeight: '800', color: '#78716c', cursor: 'pointer' }}>{t}</button>
                ))}
              </div>

              <div style={{ display: 'flex', alignItems: 'center', background: '#fdfaf5', borderRadius: '100px', padding: '10px 15px', border: '1px solid #f1ece4', gap: '10px' }}>
                {/* Hidden Input */}
                ...
                <button 
                  ...
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#c2b9af', padding: '5px' }}
                >
                  <Icons.Paperclip />
                </button>
                ...
                <button onClick={() => handleReply()} style={{ width: '45px', height: '45px', borderRadius: '50%', background: accentColor, border: 'none', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Icons.Send />
                </button>
              </div>
            </div>
      ...
      {/* 3. INTELLIGENCE PANEL (Unchanged) */}
      <div style={{ width: '320px', padding: '30px 20px', display: 'flex', flexDirection: 'column', gap: '25px' }}>
         <h4 style={{ fontSize: '11px', fontWeight: '900', color: '#c2b9af', textTransform: 'uppercase' }}>System Logic</h4>
         {activeSession && (
             <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div style={{ padding: '20px', background: '#ffffff', borderRadius: '22px', border: '1px solid #f1ece4' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontWeight: '900', fontSize: '14px' }}>
                        {locationInfo.flag && <img src={locationInfo.flag} style={{ width: '20px' }} />}
                        {locationInfo.city}, {locationInfo.country}
                    </div>
                </div>
                <div style={{ padding: '22px', background: '#fff1e6', borderRadius: '25px' }}>
                    <div style={{ fontSize: '10px', color: accentColor, fontWeight: '900', marginBottom: '8px' }}>STORE CONTEXT</div>
                    <div style={{ fontSize: '13px', color: '#8b5e3c' }}>Inquiry from <b>{currentShop}</b>.</div>
                </div>
             </div>
         )}
      </div>
